...
export default function AdminScreen() {
  ...
  const handleDelete = (collectionName, id, name) => {
    showAlert("Slett oppføring", `Er du sikker på at du vil slette ${name}?`, [
        ...
        { text: "Slett", style: "destructive", onPress: async () => {
            try { await deleteDoc(doc(db, collectionName, id)); setAlertVisible(false); } 
            catch(e) { setAlertVisible(false); showAlert("Feil", "Kunne ikke slette: " + e.message); }
        }}
    ]);
  };
  ...
  const removeFromDept = async (type, id, name) => {
    showAlert("Fjern fra avdeling", `Vil du fjerne ${name} fra ${deptName}?`, [
        { text: "Avbryt", style: "cancel", onPress: () => setAlertVisible(false) },
        { text: "Fjern", style: "destructive", onPress: async () => {
            ...
            } catch(e) { setAlertVisible(false); showAlert("Feil", "Kunne ikke fjerne: " + e.message); }
        }}
    ]);
  };
  ...
  const handleSaveChild = async () => {
    ...
    try {
      ...
      if (editId) { await updateDoc(doc(db, "children", editId), data); showAlert('Oppdatert', `${childName} er endret!`, [{text:"OK", onPress:()=>{setAlertVisible(false); resetForm()}}]); } 
      else { await addDoc(collection(db, "children"), { ...data, status: 'home', isSick: false, checkInTime: null, createdAt: new Date() }); showAlert('Suksess', `${childName} lagret!`, [{text:"OK", onPress:()=>{setAlertVisible(false); resetForm()}}]); }
  ...
  const handleSaveEmployee = async () => {
    ...
    try {
      ...
      if (editId) { await updateDoc(doc(db, "employees", editId), data); showAlert('Oppdatert', `${empName} er endret!`, [{text:"OK", onPress:()=>{setAlertVisible(false); resetForm()}}]); } 
      else { await addDoc(collection(db, "employees"), { ...data, createdAt: new Date() }); showAlert('Suksess', `${empName} lagret!`, [{text:"OK", onPress:()=>{setAlertVisible(false); resetForm()}}]); }
    } catch (e) { showAlert('Feil', 'Kunne ikke lagre.'); } finally { setLoading(false); }
  };
  ...
  const handleSaveDepartment = async () => {
    ...
    setLoading(true);
    try {
      if (editId) { await updateDoc(doc(db, "departments", editId), { name: deptName }); showAlert('Oppdatert', `Avdeling endret til ${deptName}`, [{text:"OK", onPress:()=>{setAlertVisible(false); resetForm()}}]); } 
      else { await addDoc(collection(db, "departments"), { name: deptName, createdAt: new Date() }); showAlert('Suksess', `Avdeling ${deptName} opprettet!`, [{text:"OK", onPress:()=>{setAlertVisible(false); resetForm()}}]); }
    } catch (e) { showAlert('Feil', 'Kunne ikke lagre.'); } finally { setLoading(false); }
  };
  ...
  const renderDashboard = () => {
    ...
    return (
      <ScrollView contentContainerStyle={{paddingBottom: 40}}>
        <Text style={styles.sectionHeader}>Nøkkeltall</Text>
        <View style={styles.statsGrid}>
          {/* BARN TOTALT */}
          <View style={[styles.statCard, {backgroundColor: '#e0e7ff'}]}>
            <Text style={[styles.statNumber, {color: '#4338ca'}]}>{stats.totalChildren}</Text>
            <Text style={styles.statLabel}>Barn totalt</Text>
          </View>
          
          {/* TIL STEDE */}
          <View style={[styles.statCard, {backgroundColor: '#dcfce7'}]}>
            <Text style={[styles.statNumber, {color: '#15803d'}]}>{stats.presentChildren}</Text>
            <Text style={styles.statLabel}>Til stede</Text>
          </View>
          
          {/* MED ALLERGI (Erstattet Sykmeldte) */}
          <View style={[styles.statCard, {backgroundColor: '#fee2e2'}]}>
            <Text style={[styles.statNumber, {color: '#991b1b'}]}>{stats.allergyChildren}</Text>
            <Text style={styles.statLabel}>Med Allergi</Text>
          </View>
          
          {/* ANSATTE */}
          <View style={[styles.statCard, {backgroundColor: '#ffedd5'}]}>
            <Text style={[styles.statNumber, {color: '#c2410c'}]}>{stats.totalEmployees}</Text>
            <Text style={styles.statLabel}>Ansatte</Text>
          </View>
        </View>

        <Text style={[styles.sectionHeader, {marginTop: 20}]}>Fordeling per avdeling</Text>
        {stats.deptStats.map((dept, index) => (
          <View key={index} style={styles.deptStatRow}>
            <Text style={styles.deptStatName}>{dept.name}</Text>
            <View style={styles.deptStatBadge}>
              <Text style={styles.deptStatCount}>{dept.count} barn</Text>
            </View>
          </View>
        ))}
      </ScrollView>
            ...
            <View style={styles.modalBox}>
                ...
                <View style={styles.modalButtons}>
                    {alertConfig.buttons.map((btn, index) => (
                        <TouchableOpacity key={index} style={[styles.modalBtn, btn.style === 'cancel' ? styles.modalBtnCancel : btn.style === 'destructive' ? styles.modalBtnDestructive : styles.modalBtnOk]} onPress={btn.onPress}>
                            <Text style={[styles.modalBtnText, btn.style === 'cancel' ? {color: '#374151'} : {color: 'white'}]}>{btn.text}</Text>
                        </TouchableOpacity>
                    ))}
                </View>
            </View>
  ...
  // render listeelementer
  const renderListItem = ({ item }) => (
    <View style={styles.listItem}>
      <View style={{flex: 1}}>
        <Text style={styles.itemName}>{item.name} {item.allergy && <Text style={{color: '#dc2626', fontSize: 12}}> (Allergi)</Text>}</Text>
        <Text style={styles.itemSub}>{item.avdeling || item.department}</Text>
        {item.email && <Text style={styles.itemDetail}>{item.email}</Text>}
      </View>
      <View style={styles.actionIcons}>
        <TouchableOpacity onPress={() => handleEdit(item, activeTab)} style={{marginRight: 15}}><Ionicons name="pencil" size={24} color="#4f46e5" /></TouchableOpacity>
        <TouchableOpacity onPress={() => handleDelete(activeTab === 'child' ? 'children' : 'employees', item.id, item.name)}><Ionicons name="trash-outline" size={24} color="#dc2626" /></TouchableOpacity>
      </View>
    </View>
  );
    ...
    return (
      <View style={styles.deptCard}>
        <View style={styles.deptHeader}>
            ...
            <View style={styles.actionIcons}>
                <TouchableOpacity onPress={() => handleEdit(item, 'department')} style={{marginRight: 15}}><Ionicons name="pencil" size={20} color="#4f46e5" /></TouchableOpacity>
                <TouchableOpacity onPress={() => handleDelete('departments', item.id, item.name)}><Ionicons name="trash-outline" size={20} color="#dc2626" /></TouchableOpacity>
            </View>
        </View>
    ...
    return (
        <View style={{marginTop: 20}}>
            ...
            {linkedEmployees.length > 0 ? linkedEmployees.map(e => (
                <View key={e.id} style={styles.linkedItemRow}><Text style={styles.linkedItemText}>{e.name}</Text>
                <TouchableOpacity onPress={() => removeFromDept('employee', e.id, e.name)}><Ionicons name="close-circle" size={20} color="#dc2626" /></TouchableOpacity></View>
            ...
            {linkedChildren.length > 0 ? linkedChildren.map(c => (
                <View key={c.id} style={styles.linkedItemRow}><Text style={styles.linkedItemText}>{c.name}</Text>
                <TouchableOpacity onPress={() => removeFromDept('child', c.id, c.name)}><Ionicons name="close-circle" size={20} color="#dc2626" /></TouchableOpacity></View>
            )) : <Text style={styles.emptySub}>Ingen</Text>}
        </View>
    );
  };
    ...
    >
      ...
      <View style={styles.mainContent}>
        <View style={styles.tabContainer}>
            {['dashboard', 'child', 'employee', 'department'].map(tab => (
                <TouchableOpacity key={tab} style={[styles.tabButton, activeTab === tab && styles.activeTab]} onPress={() => { setActiveTab(tab); resetForm(); }}>
                    <Ionicons 
                        name={tab === 'dashboard' ? 'stats-chart' : tab === 'child' ? "happy-outline" : tab === 'employee' ? "people-outline" : "business-outline"} 
                        size={20} color={activeTab === tab ? '#4f46e5' : '#6b7280'} 
                    />
                    <Text style={[styles.tabText, activeTab === tab && styles.activeTabText]}>
                        {tab === 'dashboard' ? 'Oversikt' : tab === 'child' ? 'Barn' : tab === 'employee' ? 'Ansatte' : 'Avd.'}
                    </Text>
                </TouchableOpacity>
            ))}
        </View>
        ...
        <View style={{flex: 1, paddingHorizontal: 20}}>
            {activeTab === 'dashboard' ? renderDashboard() : !showForm ? (
            <>
                ...
                <TouchableOpacity style={styles.fab} onPress={() => { setEditId(null); setShowForm(true); }}>
                    <Ionicons name="add" size={30} color="white" /><Text style={styles.fabText}>{activeTab === 'department' ? 'Ny Avdeling' : activeTab === 'child' ? 'Nytt Barn' : 'Ny Ansatt'}</Text>
                </TouchableOpacity>
            </>
            ) : (
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{flex:1}}>
                ...
                <View style={styles.formHeader}>
                    <TouchableOpacity onPress={resetForm} style={{padding: 10}}><Ionicons name="arrow-back" size={24} color="#374151" /></TouchableOpacity>
                    <Text style={styles.title}>{editId ? 'Rediger' : 'Ny'} {activeTab === 'child' ? 'Barn' : activeTab === 'employee' ? 'Ansatt' : 'Avdeling'}</Text>
                </View>

                {activeTab === 'department' && (<><View style={styles.formGroup}><Text style={styles.label}>Navn</Text><TextInput style={styles.input} value={deptName} onChangeText={setDeptName} /></View>{editId && renderMembersInEdit()}<TouchableOpacity style={[styles.saveBtn, {backgroundColor: '#7c3aed', marginTop: 30}]} onPress={handleSaveDepartment} disabled={loading}><ActivityIndicator animating={loading} color="#fff" style={{position:'absolute'}} /><Text style={styles.saveBtnText}>{editId ? 'OPPDATER' : 'OPPRETT'} AVDELING</Text></TouchableOpacity></>)}
                ...
                <>
                  ...
                  <View style={[styles.row, {justifyContent: 'space-between', marginBottom: 15}]}>
                      <Text style={[styles.label, {marginBottom:0}]}>Har allergi?</Text>
                      <Switch 
                        value={childAllergy} 
                        onValueChange={setChildAllergy}
                        trackColor={{ false: "#767577", true: "#ef4444" }}
                        thumbColor={childAllergy ? "#fca5a5" : "#f4f3f4"}
                      />
                  </View>

                  <View style={styles.divider} /><Text style={styles.label}>Foresatte (E-post)</Text><View style={styles.row}><TextInput style={[styles.input, {flex: 1, marginBottom: 0}]} value={emailInput} onChangeText={setEmailInput} autoCapitalize="none" keyboardType="email-address"/><TouchableOpacity style={styles.addBtn} onPress={addGuardianEmail}><Ionicons name="add" size={24} color="white" /></TouchableOpacity></View><View style={styles.chipContainer}>{guardianEmails.map((e, i) => (<View key={i} style={styles.chip}><Text style={styles.chipText}>{e}</Text><TouchableOpacity onPress={() => removeGuardianEmail(e)}><Ionicons name="close-circle" size={18} color="#ef4444" /></TouchableOpacity></View>))}</View><TouchableOpacity style={styles.saveBtn} onPress={handleSaveChild} disabled={loading}><ActivityIndicator animating={loading} color="#fff" style={{position:'absolute'}} /><Text style={styles.saveBtnText}>{editId ? 'OPPDATER' : 'OPPRETT'} BARN</Text></TouchableOpacity></>
                )}
                
                {activeTab === 'employee' && (<><View style={styles.formGroup}><Text style={styles.label}>Navn</Text><TextInput style={styles.input} value={empName} onChangeText={setEmpName} /></View><View style={styles.formGroup}><Text style={styles.label}>Avdeling</Text><TextInput style={styles.input} value={empDept} onChangeText={setEmpDept} /></View><View style={styles.formGroup}><Text style={styles.label}>E-post</Text><TextInput style={styles.input} value={empEmail} onChangeText={setEmpEmail} autoCapitalize="none" keyboardType="email-address" /></View><View style={styles.formGroup}><Text style={styles.label}>Telefon</Text><TextInput style={styles.input} value={empPhone} onChangeText={setEmpPhone} keyboardType="phone-pad" /></View><TouchableOpacity style={[styles.saveBtn, {backgroundColor: '#4338ca'}]} onPress={handleSaveEmployee} disabled={loading}><ActivityIndicator animating={loading} color="#fff" style={{position:'absolute'}} /><Text style={styles.saveBtnText}>{editId ? 'OPPDATER' : 'OPPRETT'} ANSATT</Text></TouchableOpacity></>)}
                </ScrollView>
            </KeyboardAvoidingView>
            )}
        </View>
...
const styles = StyleSheet.create({
  container: { flex: 1 }, 
  mainContent: { flex: 1, backgroundColor: '#f9fafb' },
  ...
  tabText: { marginLeft: 4, fontWeight: '600', color: '#6b7280', fontSize: 11 },
  ...
  deptStatRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'white', padding: 16, borderRadius: 12, marginBottom: 10, shadowColor: '#000', shadowOpacity: 0.05, elevation: 1 },
  deptStatName: { fontSize: 16, fontWeight: '600', color: '#1f2937' },
  deptStatBadge: { backgroundColor: '#f3f4f6', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 20 },
  deptStatCount: { fontSize: 14, fontWeight: 'bold', color: '#4b5563' },
