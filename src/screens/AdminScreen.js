...
export default function AdminScreen() {
  ...
  // state som holder styr på hvilken fane som er aktiv
  ...
  const getStats = () => {
    ...
    const sickChildren = childrenList.filter(c => c.isSick).length;
    
    // Beregn barn per avdeling
    ...
    return { totalChildren, totalEmployees, presentChildren, sickChildren, deptStats };
  };
  ...
  const handleSaveChild = async () => {
    ...
    setLoading(true);
    try {
      const data = { name: childName, avdeling: childDept, guardianEmails, image: `https://api.dicebear.com/7.x/avataaars/png?seed=${childName}` };
      ...
      <ScrollView contentContainerStyle={{paddingBottom: 40}}>
        ...
        <View style={styles.statsGrid}>
          ...
          <View style={[styles.statCard, {backgroundColor: '#fee2e2'}]}>
            <Text style={[styles.statNumber, {color: '#991b1b'}]}>{stats.sickChildren}</Text>
            <Text style={styles.statLabel}>Sykmeldte</Text>
          </View>
  ...
  const renderListItem = ({ item }) => (
    <View style={styles.listItem}>
      ...
      <View style={styles.actionIcons}>
        ...
        <TouchableOpacity onPress={() => handleDelete(activeTab === 'child' ? 'children' : 'employees', item.id, item.name)}><Ionicons name="trash-outline" size={24} color="#ef4444" /></TouchableOpacity>
      </View>
    </View>
  );
      ...
      <View style={styles.deptCard}>
        <View style={styles.deptHeader}>
            ...
            <View style={styles.actionIcons}>
                ...
                <TouchableOpacity onPress={() => handleDelete('departments', item.id, item.name)}><Ionicons name="trash-outline" size={20} color="#ef4444" /></TouchableOpacity>
            </View>
        </View>
    ...
    return (
        <View style={{marginTop: 20}}>
            ...
            {linkedEmployees.length > 0 ? linkedEmployees.map(e => (
                ...
                <TouchableOpacity onPress={() => removeFromDept('employee', e.id, e.name)}><Ionicons name="close-circle" size={20} color="#ef4444" /></TouchableOpacity></View>
            ...
            {linkedChildren.length > 0 ? linkedChildren.map(c => (
                ...
                <TouchableOpacity onPress={() => removeFromDept('child', c.id, c.name)}><Ionicons name="close-circle" size={20} color="#ef4444" /></TouchableOpacity></View>
            )) : <Text style={styles.emptySub}>Ingen</Text>}
        </View>
    );
  };
            ...
            <ScrollView contentContainerStyle={{paddingBottom: 40}}>
              ...
              {activeTab === 'department' && (
                <>
                    ...
                    <TouchableOpacity style={[styles.saveBtn, {backgroundColor: '#7c3aed', marginTop: 30}]} onPress={handleSaveDepartment} disabled={loading}><ActivityIndicator animating={loading} color="#fff" style={{position:'absolute'}} /><Text style={styles.saveBtnText}>{editId ? 'OPPDATER' : 'OPPRETT'} AVDELING</Text></TouchableOpacity>
                </>
              )}
              ...
              {activeTab === 'child' && (
                <>
                  ...
                  <View style={styles.chipContainer}>{guardianEmails.map((e, i) => (<View key={i} style={styles.chip}><Text style={styles.chipText}>{e}</Text><TouchableOpacity onPress={() => removeGuardianEmail(e)}><Ionicons name="close-circle" size={18} color="#ef4444" /></TouchableOpacity></View>))}</View>
                  <TouchableOpacity style={styles.saveBtn} onPress={handleSaveChild} disabled={loading}><ActivityIndicator animating={loading} color="#fff" style={{position:'absolute'}} /><Text style={styles.saveBtnText}>{editId ? 'OPPDATER' : 'OPPRETT'} BARN</Text></TouchableOpacity>
                </>
              )}
              ...
              {activeTab === 'employee' && (
                <>
                  ...
                  <TouchableOpacity style={[styles.saveBtn, {backgroundColor: '#4338ca'}]} onPress={handleSaveEmployee} disabled={loading}><ActivityIndicator animating={loading} color="#fff" style={{position:'absolute'}} /><Text style={styles.saveBtnText}>{editId ? 'OPPDATER' : 'OPPRETT'} ANSATT</Text></TouchableOpacity>
                </>
              )}
            </ScrollView>
          </KeyboardAvoidingView>
